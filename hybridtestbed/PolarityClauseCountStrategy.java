...
public class PolarityClauseCountStrategy extends WalkSatStrategy {
	public PolarityClauseCountStrategy(WalkSatSolver solver, ILits vocabulary) {
	...
	public boolean run(int[][] clauses, int numVars, 
		...
		for (int i = 0; i < size[hardestClause]; i++) {
			...
			int count = dataInfo.getPolarityCount(hardestClauseLiterals[i]);
			if(count > maxCount) {
				maxCount = count;
